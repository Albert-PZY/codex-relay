...
|----|-------------|-----------|---------------|-----------------|
| AC-001 | The package declares Node.js 22-only support and npm package management with pnpm compatibility. | Logic | `package.json` exists. | `engines.node` is `>=22 <23`, `package-lock.json` supports npm CI, and pnpm lock/workspace files may remain for compatibility. |
...
| AC-010 | GitHub Actions verify and publish the package through release automation. | Logic | Workflow files exist. | CI runs `npm ci`, lint, test, build, and pack; Release Please creates release tags and publishes npm with provenance after a release PR is merged. |
...
| AC-012 | Health state protects failed credentials without active probing cost. | Logic | Unit tests create cooldown and retired records. | Cooling `baseUrl + apiKey` credentials are skipped for 30 minutes, successful credentials recover and reset cooldown counts, and credentials that reach 10 cooldowns are retired. |
...
| AC-019 | The final project passes local verification. | API | Dependencies are installed. | `npm run lint`, `npm test`, `npm run build`, and `npm pack --dry-run` exit with code 0; pnpm equivalents remain supported. |
