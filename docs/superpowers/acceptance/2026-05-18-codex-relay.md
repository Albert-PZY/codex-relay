...
| AC-006 | Runner builds Codex arguments without mutating user Codex config files. | Logic | Unit tests call argument/env builders. | Env contains `OPENAI_API_KEY`, args contain `-c openai_base_url=...`, and no CODEX_HOME override is added. |
...
| AC-010 | GitHub Actions verify and publish the package through release automation. | Logic | Workflow files exist. | CI runs pnpm install, lint, test, build, and pack; npm publish workflow runs on release publish with provenance. |
